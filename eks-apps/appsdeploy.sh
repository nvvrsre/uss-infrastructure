...
echo "Copy the above LoadBalancer IPs and create DNS A records for the following domains in Godaddy:"

sleep 60


echo "👉 Deleting Ingress manifests..."

kubectl delete -f ushasree-ingress.yaml 
kubectl delete -f sonarqube-ingress.yaml 
kubectl delete -f grafana-ingress.yaml 
kubectl delete -f argocd-ingress.yaml 

echo "👉 Re-applying Ingress manifests to ensure proper setup..."
kubectl apply -f ushasree-ingress.yaml
kubectl apply -f sonarqube-ingress.yaml
kubectl apply -f grafana-ingress.yaml
kubectl apply -f argocd-ingress.yaml
